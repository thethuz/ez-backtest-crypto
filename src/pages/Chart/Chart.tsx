  ...
  const onPositionStatusChange = ({
    ...
    if (type && status === "OPENED") {
      ...
      chartDB.createPosition({
        ...
        exchange: currentExchange,
        openTime :new Date(position.openTime).toLocaleDateString() +" "+ new Date(position.openTime).toLocaleTimeString(),
        closeTime :new Date(lastCandle.timestamp).toLocaleDateString() +" "+ new Date(lastCandle.timestamp).toLocaleTimeString(),
        symbol: currentSymbol,
      });
